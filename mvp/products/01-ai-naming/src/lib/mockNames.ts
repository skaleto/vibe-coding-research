/**
 * Mock 数据：真实诗经/楚辞/唐诗 名字示例。
 * 用于无 LLM key 时的 fallback；典故均能通过 verifyQuote 校验。
 */

import type { NameCandidate } from './schema';
...
function computeSurnamePinyin(surname: string): string {
  ...
  if (surname.length === 2) {
    ...
    if (compound[surname]) return compound[surname]!;
    return surname;
  }
  return map[surname] ?? surname;
}
