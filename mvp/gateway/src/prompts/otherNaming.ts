/**
 * 公司 / 笔名 / 宠物 / 网名 Prompt —— 1:1 移植自
 * mvp/products/01-ai-naming/lib/prompts/other-naming.ts
 */

import type { BabyNamingInputs } from './babyNaming';
...
export function getSystemPromptByType(type: Exclude<NamingType, 'baby'>): string {
  switch (type) {
    case 'company':
    ...
    default:
      throw new Error(`未知类型: ${type as string}`);
  }
}

export function buildOtherNamingUserPrompt(
  type: Exclude<NamingType, 'baby'>,
  inputs: BabyNamingInputs,
): string {
